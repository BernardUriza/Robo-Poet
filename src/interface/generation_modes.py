...
class GenerationModes:
    ...
    def quick_generation(self, generator) -> None:
        ...
        print("\n[LAUNCH] GENERACIÓN RÁPIDA - PRESETS OPTIMIZADOS")
        print("=" * 50)
        
        presets = {
            '1': {'name': ' Narrativa', 'temp': 0.7, 'length': 300, 'seed': 'Once upon a time'},
            '2': {'name': '[ART] Creativo', 'temp': 1.0, 'length': 250, 'seed': 'The secret to'},
            '3': {'name': '[SCIENCE] Experimental', 'temp': 1.3, 'length': 200, 'seed': 'In this world'},
            '4': {'name': '[BOOKS] Académico', 'temp': 0.5, 'length': 350, 'seed': 'The purpose of'},
            '5': {'name': '[DRAMA] Artístico', 'temp': 1.1, 'length': 220, 'seed': 'Behind the scenes'}
        }
        
        print("[TARGET] PRESETS DISPONIBLES:")
        for key, preset in presets.items():
            print(f"{key}. {preset['name']} (T={preset['temp']}, L={preset['length']})")
        
        choice = self.validator.get_menu_choice("\n[TARGET] Selecciona preset", 5)
        preset = presets[str(choice)]
        
        print(f"\n[OK] Preset seleccionado: {preset['name']}")
        print(f" Configuración: T={preset['temp']}, L={preset['length']}")
    ...
    def creative_lab(self, generator) -> None:
        ...
        print("\n[SCIENCE] LABORATORIO CREATIVO")
        print("=" * 40)
        print(" Control total sobre la generación")
        
        # Get parameters
        seed = self.validator.get_text_input(" Seed text", "The art of", 1, 200)
        temperature = self.validator.get_float_input(" Temperature", 0.8, 0.1, 2.0)
        length = self.validator.get_number_input(" Longitud", 200, 10, 1000)
        
        # Creative options
        print("\n[ART] OPCIONES CREATIVAS:")
        print("1.  Flujo normal")
        print("2. [TARGET] Generación dirigida (múltiples intentos)")
        print("3. [CYCLE] Variaciones del mismo seed")
        
        mode = self.validator.get_menu_choice("[TARGET] Selecciona modo", 3)
    ...
    def interactive_session(self, generator) -> None:
        ...
        print("\n[GAME] SESIÓN INTERACTIVA AVANZADA")
        print("=" * 50)
        ...
        while True:
            print(f"\n Config actual: T={current_temp}, L={current_length}")
            user_input = input("[TARGET] Seed o comando > ").strip()
            ...
            if user_input.startswith('temp '):
                try:
                    ...
                    if self.validator.validate_temperature_input(current_temp):
                        print(f"[OK] Temperature actualizada: {current_temp}")
                    else:
                        current_temp = 0.8
                except:
                    print("[X] Formato inválido. Uso: temp 0.8")
                continue
                ...
                try:
                    ...
                    if self.validator.validate_length_input(current_length):
                        print(f"[OK] Longitud actualizada: {current_length}")
                    else:
                        current_length = 200
                except:
                    print("[X] Formato inválido. Uso: len 200")
                continue
                ...
                if self.last_generation:
                    ...
                    print("[X] No hay generación para guardar")
                continue
                ...
                print("[OK] Estadísticas de sesión reiniciadas")
                continue
                ...
                print("[TARGET] Saliendo de sesión interactiva...")
                self._show_session_stats()
                break
    ...
    def batch_experiments(self, generator) -> None:
        ...
        print("=" * 40)
        print("[CHART] Generación sistemática para análisis comparativo")
        ...
        print("\n[SCIENCE] TIPOS DE EXPERIMENTO:")
        for key, desc in experiment_types.items():
            print(f"{key}. {desc}")
        
        exp_type = self.validator.get_menu_choice("[TARGET] Selecciona experimento", 4)
    ...
    def thematic_templates(self, generator) -> None:
        ...
        print("\n[ART] PLANTILLAS TEMÁTICAS")
        print("=" * 40)
        print("[BOOKS] Estilos literarios predefinidos")
        
        templates = {
            '1': {
                'name': '[CASTLE] Fantasía Épica',
                'seeds': ['In a kingdom far away', 'The ancient prophecy', 'Beyond the mountains'],
                'temp': 0.9, 'length': 350
            },
            '2': {
                'name': '[SCIENCE] Ciencia Ficción', 
                'seeds': ['In the year 2157', 'The spaceship landed', 'The artificial intelligence'],
                'temp': 0.8, 'length': 300
            },
            '3': {
                'name': ' Misterio/Suspenso',
        ...
        choice = self.validator.get_menu_choice("[TARGET] Selecciona plantilla", 5)
        template = templates[str(choice)]
        
        print(f"\n[OK] Plantilla: {template['name']}")
        ...
        seed_choice = self.validator.get_menu_choice("[TARGET] Selecciona seed", len(template['seeds']))
    ...
    def _generate_and_display(self, generator, seed: str, temperature: float, length: int) -> None:
        ...
        print(f"\n[ART] Generando texto...")
        print(f"    Configuración: T={temperature}, L={length}")
        print(f"    Seed: \"{seed}\"")
        print("   ⏳ Procesando...")
        ...
        # Offer save
        if self.validator.get_confirmation("[SAVE] ¿Guardar esta generación?", False):
            self._save_last_generation()
    ...
    def _directed_generation(self, generator, seed: str, temperature: float, length: int) -> None:
        ...
        attempts = self.validator.get_number_input("[CYCLE] Número de intentos", 3, 2, 5)
    ...
    def _seed_variations(self, generator, base_seed: str, temperature: float, length: int) -> None:
        ...
        print(f"\n[CYCLE] Generando variaciones de \"{base_seed}\"...")
    ...
    def _multi_seed_experiment(self, generator) -> None:
        ...
        temperature = self.validator.get_float_input(" Temperature fija", 0.8, 0.1, 2.0)
        ...
        for i, seed in enumerate(base_seeds, 1):
            ...
            print(f"   [TIME] {gen_time:.2f}s | {len(result)} chars")
            print(f"   [DOC] {result[:100]}...")
        
        # Save experiment
        filepath = self.file_manager.save_experiment_results("multi_seed", results)
        print(f"\n[OK] Experimento guardado en: {filepath}")
    ...
    def _temperature_sweep_experiment(self, generator) -> None:
        ...
        for i, temp in enumerate(temperatures, 1):
            print(f"\n Temperatura {i}/{len(temperatures)}: {temp}")
            start_time = time.time()
            ...
            print(f"   [TIME] {gen_time:.2f}s | Creatividad: {'Alta' if temp > 1.0 else 'Media' if temp > 0.8 else 'Baja'}")
            print(f"   [DOC] {result[:100]}...")
    ...
    def _length_variation_experiment(self, generator) -> None:
        ...
        for i, length in enumerate(lengths, 1):
            ...
            print(f"   [TIME] {gen_time:.2f}s | {words} palabras | {len(result)/gen_time:.0f} chars/s")
    ...
    def _matrix_experiment(self, generator) -> None:
        ...
        print("WARNING: Experimento matriz generará muchas combinaciones")
        ...
        print(f"[SCIENCE] Generando {total_combinations} combinaciones...")
        ...
        print(f"\n[OK] Experimento matriz guardado en: {filepath}")
    ...
    def _save_last_generation(self) -> None:
        ...
        print(f"[OK] Generación guardada en: {filepath}")
    ...
    def _show_session_stats(self) -> None:
        ...
        print("\n[CHART] ESTADÍSTICAS DE SESIÓN:")
        print("-" * 40)
        print(f"   [TIME] Duración de sesión: {duration}")
        print(f"   [TARGET] Generaciones: {self.session_stats['generations_count']}")
        print(f"   [DOC] Total caracteres: {self.session_stats['total_chars']:,}")
        print(f"   [FAST] Tiempo total generación: {self.session_stats['total_time']:.1f}s")
        print(f"   [CHART] Promedio por generación: {avg_time:.2f}s, {avg_chars:.0f} chars")
        print("-" * 40)
